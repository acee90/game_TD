...
### 5.2 랭킹·대시보드에서 제외하는 것 (저장은 하되 노출 안 함)

- `build.dirty === true` (개발 빌드) 또는 알 수 없는 `gitSha`
- **정상 종료가 아닌 판** — `normal_end = 0`

**중요 — `complete`와 정상 종료는 다른 개념이다** (2026-07-22 측정으로 확인):
중단(`quit`·`restart`)도 이탈 처리에서 `finishRun()`을 부르므로 `run_finished` 이벤트를
남긴다. 즉 `complete = 1`이어도 중단 판일 수 있어, 초기 설계의 필터
(`complete=1 AND build_dirty=0`)는 중단 판을 랭킹에 포함시키는 버그였다.

| 컬럼 | 뜻 | 판정 |
|---|---|---|
| `complete` | 로그가 온전한가 | `run_finished` 이벤트 존재 |
| `normal_end` | **정상 종료인가 (노출 자격)** | `finish_reason ∈ {game_over, cleared}` |

판정 규칙의 원본은 `engine/src/game/logging.ts`의 `isNormalEnd()`이고,
Worker와 대시보드가 **같은 함수를 import**한다 (로직 이중화 금지).
중단 판은 버리지 않고 저장해 **운영·밸런스 분석**에 쓴다(`idx_runs_aborted`).
...
- **기록 (2026-07-22, 용량 실측 완료 / IndexedDB 잔존 범위는 M2로 이월):**
  봇 12판 게임오버까지 측정 —
  ...
  | 도달 라운드 | 23 | 27 | 27 |
  | 이벤트 수 | 168 | 194 | 207 |
  | JSONL | 45 KB | 52 KB | 56 KB |

  라운드당 **7.1 이벤트 · 1.92 KB** → R60 클리어 외삽 **약 427 이벤트 · 115 KB**.
  D1 실제 저장은 봉투 필드가 컬럼으로 빠져 더 작다(JSONL 56 KB 런의 `payload_json` 합 22 KB).
  **인덱스 포함 한계비용은 런당 약 42 kB**(운영 D1에 5건→10건 적재 시 324→532 kB) —
  무료 500 MB에 약 12,000판, 유료 10 GB에 약 25만 판.

  **상한 확정:** `maxEvents` 20,000 → **5,000**, `maxBytes` 5 MB → **1 MB**.
  근거는 두 천장이다 — 현실 최대(427)의 12배 여유이면서, **D1 무료 플랜의 Worker
  호출당 쿼리 50개 제한**(이벤트를 200개씩 batch로 넣으므로 약 9,400개가 실질 상한) 아래.
  207 이벤트 런의 운영 업로드가 총 5쿼리로 처리되어 `201`을 받는 것까지 확인했다
  (`batch()` 1회 = 쿼리 1회).
...
- **기록 (2026-07-22, 코드 완료 · remote 미적용):** `services/game-log-worker/`
  ...
  **운영 배포 완료 (2026-07-22):** `https://game-td.outshinelee.workers.dev`.
  기존 배포는 **Pages 프로젝트**(`game-td.pages.dev`)였는데 Pages는 `main`+D1을 이 구성으로
  얹을 수 없어 **Worker로 새로 만들었다**(대시보드: 루트 디렉터리=저장소 루트,
  빌드 `npm run build`, 배포 `npx wrangler deploy`).
  remote D1 마이그레이션 적용 완료(`runs`·`run_events` 생성 확인).
  **운영 스모크 테스트:** 홈·Wiki 목록·Wiki 상세·`/game` 모두 200 ·
  없는 경로는 `text/html` 404(JSON 아님) · `/api/health` 정상 ·
  업로드 `201` → **remote D1 적재 확인**(complete=1, 표시명, 토큰 해시, `user_id` NULL) →
  재업로드 `200 duplicate` · 빈 payload `400`. 브라우저 확인: Wiki 39카드·4병과·11계열 렌더,
  타워 상세 Phaser 프리뷰 캔버스 1개, `/game?bot` 캔버스 1개로 정상 부팅, 페이지 오류 0건.
  **검증 후 스모크 데이터는 삭제**해 운영 DB를 비운 상태로 되돌렸다(runs=0, events=0).
...
- **기록 (2026-07-23, 완료):** IndexedDB v2에 `uploads` 스토어 신설(업로드 상태),
  `listPendingRunIds()`로 "이벤트는 있는데 uploaded 표시가 없는 런"을 밀린 목록으로 본다
  (별도 pending 표시를 두지 않는다). 업로드 클라이언트는 `phaser/src/lib/game/run-upload.ts`.
  **전제 실측 검증:** 게임 진입 후 이탈 → 이벤트 6건이 그대로 남고 `run_finished`·summary만
  유실됨을 확인. §4.1 설계가 성립한다.
  **E2E 검증:** ① 중단 판이 다음 방문에 자동 업로드(`status: uploaded`)되고 D1에
  `complete=0, normal_end=0`으로 적재 ② 진행 중인 판은 제외(부분 업로드 없음)
  ③ 게임오버 시 즉시 업로드 → "랭킹에 기록했습니다" 표시, D1에 `normal_end=1`·152이벤트 적재
  ④ 페이지 오류 0건. 4xx는 `failed`로 확정해 재시도 목록에서 빼고, 네트워크 오류는
  상태를 남기지 않아 다음 방문에 다시 시도한다. 이름 저장 후 재전송은 지문이 같으므로
  conflict가 아니라 표시명만 갱신된다(Worker 보완).
...
- **기록 (2026-07-23, 완료):** `GET /api/v1/ranking`(상위 N, 점수 내림차순·동점은 소요시간),
  `GET /api/v1/runs/:runId`(요약 + 선택 기록 타임라인). 노출 조건
  `normal_end = 1 AND build_dirty = 0`을 `ranking.ts`의 상수 하나로 모았다.
  화면은 랭킹 목록 + 행을 펼치면 그 판의 **최종 보드·증강 선택·골드 사용처·선택 기록**을 보여준다.
  빌드 구성은 `summary_json`(2 KB)에서 오므로 목록·요약 경로가 `run_events`를 건드리지 않는다.
  **검증:** 랭킹 API가 정상 종료 판만 반환 · 상세 4개 섹션(최종 보드 15칩, 골드 사용처,
  선택 기록 103건) 렌더 · **중단 판 상세는 404로 비공개** · 없는 런도 404 ·
  모바일 360px 가로 스크롤 없음 · 대시보드도 정상 종료 판만 표시 · 페이지 오류 0건.
  검증 중 개발 빌드(`build_dirty=1`)가 랭킹에서 제외되는 것도 실제로 확인했다.
...
| D1 용량·비용 (모든 판 저장) | **D1 유지로 확정** (2026-07-22 사용자 결정). 근거: 런당 42 kB 실측 → 무료 500 MB에 약 12,000판이고, 중단 런을 운영 분석에 쓰려면 여러 판을 가로지르는 SQL 집계가 필요해 R2보다 D1이 맞다. **이관 판단 기준: 약 8,000판(무료 한도의 65%) 도달 시 재검토** — 그때 유료 플랜(10 GB ≈ 25만 판) 또는 `run_events`만 R2로 내리고 `runs`(요약·랭킹)는 D1에 남기는 하이브리드를 고른다. 측정 전에 두 저장소를 동시에 도입하지 않는다 |
