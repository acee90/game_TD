...
> 상태: **진행 중** · 최종 갱신: 2026-07-10 (커밋 `0d3b566` 시점)
...
- [x] `HeroData.cs`: STAT_GRANT_EVERY=20, StatGrant/StatPointsFor, HP_LEVEL_MULT_GROWTH=1.0
- [x] `Balance.cs`: BossHP = 1150 × 2.15^(lvl-1)
- [ ] **검증 필요**: `Game.cs` advanceEnemies — 보스는 영웅에게 저지되지 않는다
      (`enemy.kind !== 'boss'` 조건), 허수아비는 도발(taunts)일 때만 보스를 잡는다.
      원본: `web/src/game/game.ts` advanceEnemies.
- [ ] **검증 필요**: `Hero.cs` — bought는 구매 횟수, points는 파생(get), ComputeStats에 포인트 입력.
- [ ] **미완 추정**: `GameHud.cs` 스탯 버튼 — 보유 pt · 비용 · 다음 지급량(+1/+2) 표시.
...
- [ ] 프로브: `probeCost = round(30 × 1.3^owned)`, `PROBE_MAX = 16`, buyProbe가 지수 비용 차감
- [ ] 증강 리롤: `AUGMENT_REROLL_MAX = 2`, `augmentRerollCost(used) = 12 × (used+1)`,
      `Game.rerollsUsed`(새 선택지마다 0으로), `rerollCost/canReroll/rerollAugments()`
- [ ] 가스 스킬 개조: `GAS_SKILL_DAMAGE_MULT = 1.08` (주의 — f74d907 시점엔 1.15였다가
      같은 커밋에서 1.08로 확정), `GAS_SKILL_CDR_MULT = 0.94`, `gasSkillCost = round(30 × 1.35^n)`,
      `Hero.gasSkillDamage/gasSkillCdr`, skill fold에 가스 개조 패치 합성,
      `Game.gasSkillCost/canBuyGasSkill/buyGasSkill('damage'|'cdr')`
- [ ] HUD: 프로브 버튼에 지수 비용 표시, 증강 오버레이에 리롤 버튼(비용·남은 횟수·보유 가스),
      스킬 보유 시 개조 2버튼(피해 +8% / 쿨 -6%)

### 3. `e75e246` — 영웅 몫 재조정 (미착수, 상수 2개)

- [ ] `SKILL_PER_INT` 0.06 → **0.035**
- [ ] `STAT_COST_GROWTH` 8 → **14**

### 4. 이후 웹 커밋 확인

`git log --oneline d009de2..origin/docs/gattadi-map-analysis -- web/src` 로
이 문서 작성 이후 추가된 web 변경이 있는지 확인하고 목록에 추가한다.
(`083ad93`, `0d3b566`은 문서 경로 주석 변경뿐 — Core 로직 무관, 이미 커밋됨.)
...
- [ ] 위 1~4 전 항목 체크
- [ ] 코디네이터 검수 → 커밋 (메시지에 동기화한 웹 커밋 해시 명시)
- [ ] 이 문서 상태를 "완료"로 갱신
