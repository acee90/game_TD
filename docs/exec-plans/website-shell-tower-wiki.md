...
> 상태: **현행 (M0~M7 실행 완료, 2026-07-22)** · 최종 갱신: 2026-07-22
...
- **기록 (2026-07-22, 완료):** 대시보드 = 명예의 전당 1위(최고 기록) + 완주 런
  summary 최근 5개(`listRunSummaries`, "완료된 판만 기록됩니다" 명시 — M2에서 관찰한
  quit-summary 유실 때문). 랭킹 = `hallOfFame.load()` 읽기 전용 테이블, "내 브라우저
  기록" 명시. 저장소 접근은 전부 onMount(클라이언트) — SSR 프리렌더 안전.
  검증: 빈 상태 두 페이지 확인 후, 실제 저장 키·스키마(`gattadi.hallOfFame.v1`)로
  주입한 기록이 테이블·최고 기록 카드에 렌더됨(검증 후 제거). 출고 코드에 가짜 데이터 없음.
...
- **기록 (2026-07-22, 완료):** §8 완료 기준 전 항목 통과 — 8개 경로+39 상세 직접 접근
  200 · 홈/위키 목록 문서에서 Phaser 청크(B61OQUcB, 1.48MB) 미로드, 상세·게임에서만 로드 ·
  모바일 360px 6개 화면 가로 스크롤 0 · Tab 순회·aria-current·프리뷰 aria-label 확인 ·
  reduced-motion 에뮬레이션에서 자동 재생 없음(재생 버튼 → 정상 시작) ·
  `engine npm run check` 0 에러 · Vitest 359 통과 · `phaser npm run build` 통과.
  명세 §11에 채택 구조 기록. **미검증(후속)**: 실 호스팅 배포(도메인 결정 대기),
  실기기 모바일 터치, Lighthouse 성능 계측.
