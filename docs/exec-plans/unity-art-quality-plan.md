...
> 상태: **초안** (승인 대기) · 작성일 2026-07-10 · 최종 갱신: 2026-07-17
> 범위: `unity/Assets/Scripts/View/` + 신규 커밋 에셋. **Core/는 건드리지 않는다.**
> 이 문서는 계획까지다. 구현은 승인 후 별도 진행한다.
> 전투 파티클·투사체·조합·보스 연출의 상세 계획은
> [Unity 전투 VFX 품질 계획](unity-vfx-quality-plan.md)이 맡는다.
...
**현행 (2026-07-14, [세계관 문서](../design/worldbuilding.md) 중세 판타지 전환):**
- **최종 아트 방향 — 스타일라이즈드 로우폴리 중세 판타지.** 초한지 고증 제약은 폐기됐다.
  철·가죽·목재 물성은 유지하되 룬·마법 무기와 제한적인 발광을 허용한다.
- **색을 어디에 싣는가.** 몸체 전체 도색에만 의존하지 않고, 무기 효과·깃발·천·룬처럼
  부감에서 보이는 넓은 면에 계열색을 싣는다. §2.6의 "유닛은 보드보다 밝아야 한다"는
  실측 결론은 그대로 유지한다.
- **외부 애셋 정책.** 세계관 §8: **CC0(Kenney·Quaternius)만.** 이 저장소는 공개라
  Asset Store EULA(원본 파일 재배포 금지)상 무료 팩도 커밋할 수 없다.
  유럽풍 저폴리 소품도 검토할 수 있지만 **전력 유닛과 맵 타일은 자체 제작·절차적 생성을 유지**한다.
