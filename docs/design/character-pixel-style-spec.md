...
### 2.6 FreeCharactersAnimationsAssetPack 공통 스타일 분석 [원본확정]

`resources/FreeCharactersAnimationsAssetPack/iXDJO9.gif` 미리보기와
`SpriteSheets(96x96)/Human_Soldier_Sword_Shield`, `Monster_Slime` 원본 시트를
교차 확인한 결과다. 이후 신규 영웅·궁수·마법사는 이 공통 스타일을 우선 기준으로 삼는다.

- 캐릭터는 96×96 셀 안에 작게 놓이며, 실제 게임 화면에서 약 20px급으로 읽힌다. 큰 일러스트나 치비 아이콘처럼 셀을 채우지 않는다.
- 머리·몸통·장비는 몇 개의 불투명 색 덩어리로만 나뉜다. 얼굴, 갑주, 천, 무기의 세부 묘사는 역할 판독에 필요한 최소 픽셀만 남긴다.
- 외곽선은 어두운 갈색-회색 1px 계열이며, 모든 경계를 기계적으로 두르기보다 본체 사이의 틈과 그림자 면으로 실루엣을 만든다.
- 인간형은 머리만 키우거나 몸통을 넓히지 않은 짧은 인간 비율이다. 눈·손·갑주 홈·장식은 1~2px 색 덩어리로 통합한다.
- 병과 구분은 몸체 색보다 손에 든 무기·방패·모자·망토의 외곽 돌출로 만든다. 검은 얇은 밝은 축과 작은 손잡이, 방패는 하나의 평면 덩어리로 처리한다.
- 명암은 재질마다 기본색·어두운 면·밝은 면의 2~3단계로 제한하며, 사진식 금속 반사나 반복 질감을 사용하지 않는다.
- GIF 미리보기의 회색 배경과 바닥은 미리보기용이며, 실제 유닛 리소스에는 굽지 않는다. 게임 리소스는 RGBA 투명 배경과 별도 계단형 그림자를 사용한다.
- 24×32 낱장 리소스가 필요한 경우에도 96×96 기준 캐릭터를 1/3 배율로 축소한 표현 밀도를 유지한다. 24×32 캔버스를 새 일러스트를 그리는 공간으로 해석하지 않는다.
