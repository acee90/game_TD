...
namespace GodTD.Core
{
    public static class HeroData
    {
        ...
        /// 블로킹을 사는 타입이라 그렇다.
        /// </summary>
        public const float HERO_DAMAGE_PER_LEVEL = 26f;
        public const float HERO_HP_PER_LEVEL = 90f;

        // ───────── 전직 ─────────
        // 웹 원본은 게임 시작에 영웅 타입을 골랐지만, 최신 설계는 **영웅 Lv5에 "전직"을 고른다**.
        // 전직 전에는 중립 스탯(배수 전부 1)으로 싸운다. 첫 증강(Lv9)보다 먼저 오므로
        // 증강 가중치는 언제나 전직된 타입이 정한다. [Unity 신규 — web에는 아직 시작 선택으로 남아 있다]
