...
- **HUD**: OnGUI 즉시모드 유지 — RTS 식 상단 작전 바 + 하단 통합 콘솔.
  하단은 좌측 선택 정보(영웅·타워·몹·보스) / 우측 3×3 커맨드 카드로 나누고, 금속 프레임·인셋·
  기능군 액센트를 코드로 그려 에셋 없이도 스타크래프트/워크래프트 계열의 정보 계층을 만든다.
