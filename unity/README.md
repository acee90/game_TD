...
| 선택 | 카드 내용 |
|---|---|
| **영웅** | `Q`힘 `W`민첩 `E`지능 / `A`스킬 피해 `S`쿨타임 |
| **타워** | `Q`판매 |
| **빈 타일** | `Q`유닛 생성 |
...
## 영웅 빌드 — 세 개의 곱연산 축 + 적응형 드래프트

영웅 타입(전사/궁수/마법사)과 전직은 없다. **파워 = 스탯(골드) × 레벨 배수(경험치) ×
증강 배수(선택)** — 자원마다 축이 하나씩이다.

- **스탯 구매(골드)**: 힘(공격 +1/체력 +18 per pt) · 민첩(공속 +4%/pt, 간격 하한 0.25초) ·
  지능(스킬 피해 +3.5%/pt). 기본 7/8/8, 비용은 선형 `25 + 14n` —
  구매 횟수가 아니라 **환산 포인트**가 스탯이 된다 (20번마다 지급량 +1)
- **레벨 배수(경험치)**: `1 + 2.4×(L-1)` — 스탯이 만든 값에 곱해지므로
  같은 +1힘도 후반에 사면 절대값이 몇 배로 커진다
- **증강 드래프트**: 뽑기 가중치는 적응형 —
...
- 경제: 미네랄/가스/프로브(지수 비용 30×1.3^n, 상한 16), 보스 처치 보상, 킬 마일스톤,
...
- 영웅: 경로 위 이동, 어그로 블로킹, 부활. 힘/민첩/지능 스탯 구매(25+14n, 20번마다
  지급 포인트 +1) × 레벨 배수(1+2.4×(L-1), XP 비용은 지수 14×1.06^L) × 증강 배수,
  스킬 피해에 지능 반영(+3.5%/pt)
