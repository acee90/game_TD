...
// - 초반 템포(earlyTempo)로 R1~4는 몹 체력이 ×0.6~0.9로 낮게 스폰된다 — 반영.
...
/** 보스 소환 간격(라운드) — 쿨타임 45초 ÷ 라운드 22초 */
const BOSS_EVERY = B.BOSS_COOLDOWN_SECONDS / B.ROUND_SECONDS;
...
export function runBand(z: number, maxRound = 70): RoundRow[] {
  ...
  for (let r = 1; r <= maxRound; r++) {
    ...
    gold += income;
    gas += probes * B.GAS_PER_PROBE_SECOND * B.ROUND_SECONDS;
    ...
    const required = (B.waveTotalHp(r) * B.earlyTempo(r)) / exposureSeconds(r);
...
export function passProbability(round: number, band: RoundRow[]): number {
  ...
  const required = (B.waveTotalHp(round) * B.earlyTempo(round)) / exposureSeconds(round);
