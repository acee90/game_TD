...
/**
 * 자동 시전·낙점 셈법에서 **보스는 3기 취급** (2026-07-21, 사용자 지시).
 *
 * 밀집도(몇 기가 맞나)로만 세면 홀로 걷는 보스는 밀집도 1이라 ① 최소 대상
 * 조건(autoCastMinTargets ≥ 2)을 영영 못 채우고 ② 낙점이 항상 잡몹 무리에 밀렸다 —
 * 보스 소환이 "순수 화력 시험"인데 주력 스킬이 보스를 무시하는 설계 구멍.
 * 3이면 혼자 있는 보스도 대부분의 스킬 시전 조건을 채우되, 뭉친 잡몹(10기+)이
 * 있으면 여전히 그쪽이 우선이다 — 낙점 우선권은 별도 규칙(경로에서 앞선 보스 우선,
 * game.ts bossAheadOf)이 가진다.
 */
export const BOSS_TARGET_WEIGHT = 3;
