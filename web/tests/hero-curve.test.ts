...
// 목표 (2026-07-17 4차): 수입 20% 투자로 R45에 Lv ~24 · 실질 상한 ~Lv43.
...
describe('경험치 비용 — 지수', () => {
  ...
  test('실질 상한 ≈ Lv40대 초반 — R60 고점 수입(8,572) 전액으로도 Lv45는 못 산다', () => {
    expect(xpToLevel(45)).toBeGreaterThan(8572);
  });
});

describe('레벨 속도 — 골드가 주 연료다 (설계: 수입 20%로 R45에 Lv ~24)', () => {
  ...
  test('수입 저점의 ~20%를 XP에 넣으면 R45에 Lv 21~26', () => {
    const hero = new Hero();
    ...
    expect(hero.level).toBeGreaterThanOrEqual(21);
    expect(hero.level).toBeLessThanOrEqual(26);
  });
});
